...
func InitRouters(app *iris.Application) {
	...
	if panel.Current != nil {
		...
		app.Get("/player.css", handler)
		app.Get("/player.js", handler)
		app.Get("/mpegts.min.js", handler)
		app.Get("/hls.min.js", handler)
		app.Get("/api/stream/proxy", iris.FromStd(http.HandlerFunc(panel.Current.ServeStreamProxy)))
		app.Options("/api/stream/proxy", iris.FromStd(http.HandlerFunc(panel.Current.ServeStreamProxy)))
